...
def play():
	print("\n==========QUIZ START==========")
	score = 0
	with open("shared/questions.json", 'r+') as f:
		j = json.load(f)
		for i in range(10):
...
def createAccount():
	...
	with open('shared/users.json', 'r+') as user_accounts: #The r+ mode is used to open a file for both reading and writing
...
def loginAccount():
	...
	with open('shared/users.json', 'r') as user_accounts:
		users = json.load(user_accounts)
